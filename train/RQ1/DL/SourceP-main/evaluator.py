...
def read_predictions(filenames):
    ...
    for filename in filenames:
        ...
        with open(filename) as f:
            for line in f:
                line = line.strip()
                idx1, score_str = line.split()
                if 'txt' in line:
                    idx1 = idx1.split('/')[-1][:-4]
                prediction[(idx1)] = float(score_str)
...
def calculate_scores(answers, predictions):
    ...
    for prediction in predictions:
        y_trues_str, y_scores = [], []
        for key in answers:
            if key not in prediction:
                logging.error(
                    "Missing prediction for ({},{}) pair.".format(key[0], key[1]))
                sys.exit()
            y_trues_str.append(answers[key])
            y_scores.append(prediction[key])

        y_trues = [int(x) for x in y_trues_str]
        ...
        score['ROC-AUC'] = roc_auc_score(
            y_trues, y_scores, average='macro')
        score['AUC-PR'] = average_precision_score(
            y_trues, y_scores, average='macro')
