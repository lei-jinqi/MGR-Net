...
from source_parser import (remove_comments_and_docstrings,
                           tree_to_token_index,
                           index_to_code_token,
                           tree_to_variable_index)
from source_parser import DFG_solidity
...
for lang in dfg_function:
    LANGUAGE = Language('source_parser/my-languages.so', lang)
    parser = Parser()
...
def evaluate(args, model, tokenizer, eval_when_training=False):
    ...
    roc_auc = roc_auc_score(y_trues, y_scores, average='macro')
    auc_pr = average_precision_score(y_trues, y_scores, average='macro')
...
def test(args, model, tokenizer, best_threshold=0):
    ...
    with open(os.path.join(args.output_dir, "predictions.txt"), 'w') as f:
        for example, score in zip(eval_dataset.examples, y_scores):
            f.write(example.url1 + '\t' + str(score) + '\n')
