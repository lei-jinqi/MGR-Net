...
class AverageFusion(nn.Module):
    ...
    def forward(self, op_vec, cfg_vec, dfg_vec):
        averaged = (op_vec + cfg_vec + dfg_vec) / 3.0
        return averaged


class PonziDetectorAblation(nn.Module):
    def __init__(self, op_vocab_size, dfg_vocab_size, config, fusion_mode='Gated'):
        super().__init__()
        self.config = config
        ...
        classifier_input_dim = config.FUSION_DIM

        if self.fusion_mode == 'Gated':
            self.fusion = GatedFusion(config.LSTM_HIDDEN_DIM, config.FUSION_DIM)
            classifier_input_dim = config.FUSION_DIM
        elif self.fusion_mode == 'Concat':
            self.fusion = ConcatFusion(config.LSTM_HIDDEN_DIM, config.FUSION_DIM)
            classifier_input_dim = config.LSTM_HIDDEN_DIM * 3
        elif self.fusion_mode == 'Average':
            self.fusion = AverageFusion(config.LSTM_HIDDEN_DIM, config.FUSION_DIM)
            classifier_input_dim = config.LSTM_HIDDEN_DIM
        else:
...
def main():
    ...
    for mode in fusion_modes:
        print(f"\n\n{'=' * 50}")
        print(f"--- Starting {config.N_SPLITS}-Fold CV for Fusion Ablation: {mode} ---")
        print(f"{'=' * 50}")

        fold_metrics = []

        for fold, (train_val_idx, test_idx) in enumerate(kfold.split(X, y)):
            print(f"\n--- {mode} | Fold {fold + 1}/{config.N_SPLITS} ---")
            ...
            best_val_f1 = 0
            patience_counter = 0
            best_model_path = f'best_model_fusion_{mode}_fold_{fold}.pth'
    ...
    print("\n\n" + "=" * 50)
    print("--- All Fusion Ablation CV Results (Averaged ± Std) ---")
    print("=" * 50)
