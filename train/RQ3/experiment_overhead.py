...
class Tokenizer:
    ...
    def build_vocab(self, flat_corpus):
        ...
        print(f"词汇表构建完成。总Token数量 (>= {self.min_freq} 词频): {self.get_vocab_size()}")
...
def get_gpu_memory_usage(device):
    ...
    if device.type == 'cuda':
        ...
        return allocated_mb, percentage
    return 0.0, 0.0


def train_epoch(model, dataloader, optimizer, criterion, device, handle):
    model.train()
    total_loss = 0

    epoch_gpu_samples = []
    epoch_mem_mb_samples = []
    epoch_mem_pct_samples = []
    ...
    for op_data, cfg_data, dfg_data, labels in dataloader:
        ...
        if device.type == 'cuda':
            if handle:
                ...
                epoch_gpu_samples.append(util.gpu)


            mem_mb, mem_pct = get_gpu_memory_usage(device)
            epoch_mem_mb_samples.append(mem_mb)
            epoch_mem_pct_samples.append(mem_pct)

    avg_gpu = np.mean(epoch_gpu_samples) if epoch_gpu_samples else 0
    max_mem_mb = np.max(epoch_mem_mb_samples) if epoch_mem_mb_samples else 0
    max_mem_pct = np.max(epoch_mem_pct_samples) if epoch_mem_pct_samples else 0

    return total_loss / len(dataloader), avg_gpu, max_mem_mb, max_mem_pct
...
def main():
    ...
    if device.type == 'cuda':
        try:
            ...
            print(f"成功初始化 pynvml。监测 GPU: {gpu_name}")
        except Exception as e:
            print(f"警告: 无法初始化 pynvml。错误: {e}")
    ...
    try:
        ...
        print(f"错误: {e}. 请确保所有CSV文件都在正确的路径下。")
        return
    ...
    print(f"数据加载与清洗完成。最终有效合约数量: {len(df)}")
    ...
    for fold, (train_val_idx, test_idx) in enumerate(kfold.split(df, y)):
        ...
        print(f"当前折数据划分: {len(train_df)} 训练 / {len(val_df)} 验证 / {len(test_df)} 测试")
        ...
        print("\n--- 步骤 3.5: 开始训练 (测量 MTime, %GPU, %MEM) ---")
        best_val_f1 = 0
        patience_counter = 0
        best_model_path = f'best_model_fold_{fold + 1}.pth'

        fold_train_gpu_samples = []
        fold_train_mem_mb_samples = []
        fold_train_mem_pct_samples = []

        train_start_time = time.time()

        for epoch in range(config.EPOCHS):
            train_loss, epoch_gpu, epoch_mem_mb, epoch_mem_pct = train_epoch(model, train_loader, optimizer, criterion,
                                                                             device, handle)

            fold_train_gpu_samples.append(epoch_gpu)
            fold_train_mem_mb_samples.append(epoch_mem_mb)
            fold_train_mem_pct_samples.append(epoch_mem_pct)
            ...
            if epoch % 10 == 0 or epoch == config.EPOCHS - 1:
                print(
                    f"  -- Ep {epoch + 1}: Loss: {train_loss:.4f}, Val F1: {val_f1:.4f}, Mem: {epoch_mem_mb:.2f} MB ({epoch_mem_pct:.2f}%)")
            ...
            if val_f1 > best_val_f1:
                ...
                if patience_counter >= config.EARLY_STOPPING_PATIENCE:
                    print(f"  -- EarlyStopping 触发。")
                    break

        train_end_time = time.time()

        mtime_sec = train_end_time - train_start_time
        avg_train_gpu = np.mean(fold_train_gpu_samples) if fold_train_gpu_samples else 0
        avg_train_mem_mb = np.mean(fold_train_mem_mb_samples) if fold_train_mem_mb_samples else 0
        avg_train_mem_pct = np.mean(fold_train_mem_pct_samples) if fold_train_mem_pct_samples else 0

        model_size_mb = os.path.getsize(best_model_path) / (1024 * 1024) if os.path.exists(best_model_path) else 0

        print(f"\n--- 步骤 3.6: 最终测试 (折 {fold + 1}) ---")
        ...
        print(f"\n--- 步骤 3.7: 测量检测开销 (Latency & Memory) ---")
        model.eval()
        total_dtime_sec = 0
        total_contracts = 0

        detect_gpu_samples = []
        detect_mem_mb_samples = []
        detect_mem_pct_samples = []

        if device.type == 'cuda':
            torch.cuda.reset_peak_memory_stats(device)

        with torch.no_grad():
            for op_data, cfg_data, dfg_data, labels in tqdm(test_loader, desc=f"Detection Overhead"):
                op_data, labels = op_data.to(device), labels.to(device)
                cfg_data, dfg_data = cfg_data.to(device), dfg_data.to(device)
                ...
                _ = model(op_data, cfg_data, dfg_data)
                ...
                total_dtime_sec += (d_end - d_start)
                total_contracts += batch_size

               
                if device.type == 'cuda':
                    if handle:
                        detect_gpu_samples.append(pynvml.nvmlDeviceGetUtilizationRates(handle).gpu)

                    mem_mb, mem_pct = get_gpu_memory_usage(device)
                    detect_mem_mb_samples.append(mem_mb)
                    detect_mem_pct_samples.append(mem_pct)

        avg_dtime_ms = (total_dtime_sec / total_contracts) * 1000
        avg_detect_gpu = np.mean(detect_gpu_samples) if detect_gpu_samples else 0
        avg_detect_mem_mb = np.mean(detect_mem_mb_samples) if detect_mem_mb_samples else 0
        avg_detect_mem_pct = np.mean(detect_mem_pct_samples) if detect_mem_pct_samples else 0

        print(f"  DTime: {avg_dtime_ms:.4f} ms/contract")
        print(f"  MTime: {mtime_sec:.2f} s")
        print(f"  Model Size: {model_size_mb:.2f} MB")
        print(f"  Train GPU: {avg_train_gpu:.2f} % | Mem: {avg_train_mem_mb:.2f} MB ({avg_train_mem_pct:.2f}%)")
        print(f"  Detect GPU: {avg_detect_gpu:.2f} % | Mem: {avg_detect_mem_mb:.2f} MB ({avg_detect_mem_pct:.2f}%)")

        overhead_metrics.append({
            'MTime_sec': mtime_sec,
            'DTime_ms': avg_dtime_ms,
            'Train_GPU_Pct': avg_train_gpu,
            'Train_MEM_MB': avg_train_mem_mb,
            'Train_MEM_Pct': avg_train_mem_pct,
            'Detect_GPU_Pct': avg_detect_gpu,
            'Detect_MEM_MB': avg_detect_mem_mb,
            'Detect_MEM_Pct': avg_detect_mem_pct,
    ...
    print("\n\n" + "=" * 30)
    print(f"--- 最终开销平均结果 ---")
    overhead_df = pd.DataFrame(overhead_metrics)
    print(overhead_df.to_string())
    print("\n平均值 (± Std):")
    for metric in overhead_df.columns:
        mean_val = overhead_df[metric].mean()
        std_val = overhead_df[metric].std()
